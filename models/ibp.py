...
class IntervalBoundedTensor(BoundedTensor):
    ...
    def __repr__(self):
        return 'IntervalBoundedTensor(val=%s, lb=%s, ub=%s)' % (self.val, self.lb, self.ub)
...
def matmul(A: IntervalBoundedTensor, B: IntervalBoundedTensor):
    abstA_lb = A.lb
    abstA_ub = A.ub
    Albneg = abstA_lb * (abstA_ub < 0.)
    Albneu = abstA_lb * (abstA_lb <= 0.) * (abstA_ub >= 0.)
    Albpos = abstA_lb * (abstA_lb > 0.)
    Aubneg = abstA_ub * (abstA_ub < 0.)
    Aubneu = abstA_ub * (abstA_lb <= 0.) * (abstA_ub >= 0.)
    Aubpos = abstA_ub * (abstA_lb > 0.)
    abstB_lb = B.lb
    abstB_ub = B.ub
    Blbneg = abstB_lb * (abstB_ub < 0.)
    Blbneu = abstB_lb * (abstB_lb <= 0.) * (abstB_ub >= 0.)
    Blbpos = abstB_lb * (abstB_lb > 0.)
    Bubneg = abstB_ub * (abstB_ub < 0.)
    Bubneu = abstB_ub * (abstB_lb <= 0.) * (abstB_ub >= 0.)
    Bubpos = abstB_ub * (abstB_lb > 0.)

    lb = torch.matmul(Aubneg, Bubneg) + torch.matmul(Aubneu, Blbneg) + torch.matmul(Aubpos, Blbneg) + \
         torch.matmul(Albneg, Bubneu) + torch.matmul(Aubneu, Blbneu) + torch.matmul(Albneu, Bubneu) + \
         torch.matmul(Aubpos, Blbneu) + torch.matmul(Albneg, Bubpos) + torch.matmul(Albneu, Bubpos) + \
         torch.matmul(Albpos, Blbpos)
    ub = torch.matmul(Albneg, Blbneg) + torch.matmul(Albneu, Blbneg) + torch.matmul(Albpos, Bubneg) + \
         torch.matmul(Albneg, Blbneu) + torch.matmul(Albneu, Blbneu) + torch.matmul(Aubneu, Bubneu) + \
         torch.matmul(Aubpos, Bubneu) + torch.matmul(Aubneg, Blbpos) + torch.matmul(Aubneu, Bubpos) + \
         torch.matmul(Aubpos, Bubpos)

    ret_lb = torch.matmul(torch.clamp(A.lb, min=0), torch.clamp(B.lb, min=0)) + \
             torch.matmul(torch.clamp(A.ub, min=0), torch.clamp(B.lb, max=0)) + \
             torch.matmul(torch.clamp(A.lb, max=0), torch.clamp(B.ub, min=0)) + \
             torch.matmul(torch.clamp(A.ub, max=0), torch.clamp(B.ub, max=0))
    ret_ub = torch.matmul(torch.clamp(A.ub, min=0), torch.clamp(B.ub, min=0)) + \
             torch.matmul(torch.clamp(A.lb, min=0), torch.clamp(B.ub, max=0)) + \
             torch.matmul(torch.clamp(A.ub, max=0), torch.clamp(B.lb, min=0)) + \
             torch.matmul(torch.clamp(A.lb, max=0), torch.clamp(B.lb, max=0))
    # if not torch.all(ret_ub >= ub - TOLERANCE):
    #     warnings.warn(str(torch.min(ret_ub - ub)))
    # if not torch.all(ret_lb <= lb + TOLERANCE):
    #     warnings.warn(str(torch.max(ret_lb - lb)))
    # assert torch.max(torch.abs(ret_ub - ub)).detach().item() <= TOLERANCE and torch.max(
    #     torch.abs(ret_lb - lb)).detach().item() <= TOLERANCE
    # print(torch.max(torch.abs(ret_ub - ub)), torch.max(torch.abs(ret_lb - lb)))
    return IntervalBoundedTensor(torch.matmul(A.val, B.val), lb, ub)


class LinearBound(nn.Module):
    def __init__(self, in_features, out_features, bias=True, epsilon=0, bias_epsilon=0):
        super(LinearBound, self).__init__()
    ...
    def forward(self, x):
        if isinstance(x, torch.Tensor):
            x = IntervalBoundedTensor.point(x)

        if isinstance(x, IntervalBoundedTensor):
            ans = matmul(x, IntervalBoundedTensor(self.linear.weight.T, self.linear.weight.T - self.epsilon,
                                                  self.linear.weight.T + self.epsilon))
            ans = ans + self.linear.bias
            ans.lb = ans.lb - self.bias_epsilon
            ans.ub = ans.ub + self.bias_epsilon
            return ans
        else:
            raise TypeError(x)
    ...
    def forward_point_weights_bias(self, x):
        ...
        if isinstance(x, IntervalBoundedTensor):
            ...
            mu_cur = (x.ub + x.lb) / 2
            r_cur = (x.ub - x.lb) / 2
            mu_new = F.linear(mu_cur, self.linear.weight, self.linear.bias)
            r_new = F.linear(r_cur, weight_abs)
            return IntervalBoundedTensor(z, mu_new - r_new, mu_new + r_new)
        else:
            raise TypeError(x)
