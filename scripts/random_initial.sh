...
#      --seed $i 2>&1
    for ibp_approach in "${ibp_approaches[@]}"; do
      CUDA_VISIBLE_DEVICES="" python train.py IBP"$dataset"counternet"$ibp_approach""$i" --model IBP --cfx counternet \
        --epoch $epoch --epsilon $EPS --bias_epsilon $BEPS --tightness $ibp_approach --config assets/"$dataset".json \
        --seed $i --cfx_generation_freq 1 --ratio 0.2 --wandb 2>&1
      ...
      CUDA_VISIBLE_DEVICES="" python eval.py IBP"$dataset"counternet"$ibp_approach""$i" --cfx counternet --log_name \
        IBP"$dataset"counternet"$ibp_approach""$i".log --epsilon $EPS --bias_epsilon $BEPS --config assets/"$dataset".json \
        --seed $i >/dev/null 2>&1 &
    done
  done
done
