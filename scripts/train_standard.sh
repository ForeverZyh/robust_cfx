...
do
    ...
    python train.py $modelname $dataset --save_dir $modeldir --model Standard --epoch $epochs --seed $i

    # if dataset==who or dataset==ctg need to finetune
    if [ $dataset == "who" ] || [ $dataset == "ctg" ]; then
        epochs=20
        python train.py $modelname $dataset --save_dir $modeldir --model Standard --epoch $epochs --finetune --seed $i
    fi
done
...
# for i in {0..9} 
# do
#     modelname=$dataset"Standard_LOO"$i 

#     if [ $dataset == "who" ] || [ $dataset == "ctg" ]; then
#         exit 1
#     fi

#     python train.py $modelname $dataset --save_dir $modeldir --model Standard --epoch $epochs --seed 0 --remove_pct 1 --removal_start $i
